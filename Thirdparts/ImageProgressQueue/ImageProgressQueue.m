...
- (void)addOperation:(ImageProgress*)op {
    if (operation && operation.indexPath.row == op.indexPath.row && operation.indexPath.section == op.indexPath.section && operation.tag == op.tag) {
        return;
    }
    BOOL exist = NO;
    for (ImageProgress * opt in queue) {
        if (opt.indexPath.row == op.indexPath.row && opt.indexPath.section == op.indexPath.section && opt.tag == op.tag) {
...
- (void)imageProgress:(ImageProgress*)sender completed:(BOOL)bl {
    if (bl && sender.loaded) {
        ...
        } else if ([self.delegate respondsToSelector:@selector(imageProgressCompleted:indexPath:tag:url:)]) {
            UIImage * img = sender.image;
            [self.delegate imageProgressCompleted:img indexPath:sender.indexPath tag:sender.tag url:sender.imageURLString];
        }
    }
    [self executeNext];
}
