...
- (id)initWithOrigin:(CGPoint)point del:(id)del {
    if (self = [super initWithFrame:CGRectMake(point.x, point.y, 320.0f, 216.0f)]) {
        self.delegate = del;
        [self initDefault];
    }
    return self;
...
- (void)generateInputView {
    NSMutableArray * emojiKeys = [NSMutableArray array];
    int emojiRangeArray[] = {0xE001+85,0xE001+88,0xE101+4,0xE101+7,0xE401,0xE401+23,0xE401+24,0xE401+33,0xE001+12,0xE001+17,0xE001,0xE001+11,0xE001+18,0xE001+84,0xE05A,0xE05A,0xE101,0xE101+3,0xE101+8,0xE15A,0xE201,0xE253,0xE301,0xE34D,0xE401+34,0xE44C,0xE501,0xE537};//28
    for (int j = 0; j < 28; j += 2) {
        int startIndex = emojiRangeArray[j];
        int endIndex = emojiRangeArray[j+1];
        for (unsigned short i = startIndex ; i <= endIndex ; i++ ) {
            [emojiKeys addObject:[NSNumber numberWithUnsignedShort:i]];
        }
    }
    ...
    [self addKeysOnPad0:emojiKeys];
    [self addSubview:keyPad0];
}

- (void)addKeysOnPad0:(NSArray*)keys {
    int pages = 1;
    int height = 0;
	...
	for (int n = 0; n < keys.count; n++) {
        ...
        unsigned short keyValue = [[keys objectAtIndex:n] unsignedShortValue];
        NSString * imgName = [EmotionInputView emojiChar4To5:[NSString stringWithFormat:@"%C", keyValue]];
    ...
    pageC.center = CGPointMake(160, 180);
...
- (void)keyEmotionClick:(UIButton*)sender {
    NSString * str = [NSString stringWithFormat:@"%C", (unsigned short)sender.tag];
    if ([delegate respondsToSelector:@selector(emotionInputView:output:)]) {
        [delegate emotionInputView:self output:str];
    }
}
...
+ (NSString*)emojiCodeToEMJ:(NSString*)acode {
    ...
    if ((val >= 0xE001 && val <= 0xE05A) || (val >= 0xE101 && val <= 0xE15A) || (val >= 0xE201 && val <= 0xE253) || (val >= 0xE301 && val <= 0xE34D) || (val >= 0xE401 && val <= 0xE44C) || (val >= 0xE501 && val <= 0xE537)) {
